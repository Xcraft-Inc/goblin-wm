...
const {dialog, ipcMain, BrowserWindow, screen, session} = require('electron');
...
//MAPPING WM@ <-> INSTANCE (MUST BE SHARED OUTSIDE GOBLIN'S instance FOR MODAL PURPOSE)
...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  const onJitter = (payload) => {
    ...
    if (channel) {
      let action;
      if (Object.prototype.hasOwnProperty.call(payload, 'jitter')) {
        action = {type: 'PUSH_JITTER', ...payload};
      } else {
        action = {type: 'NO_JITTER', ...payload};
      }
      channel.sendAction(action);
    }
  };
  ...
  if (modal && parentId) {
    winOptions.parent = WINDOWS_REGISTRY[parentId];
    winOptions.modal = true;
  }

  //CREATE&REGISTER WINDOW INSTANCE
  const win = new BrowserWindow(winOptions);
  WINDOWS_REGISTRY[quest.goblin.id] = win;
  ...
  const attempt = {};
  const jitterTimeout = {};
  const unsub = quest.sub.local('greathall::<jitter>', (err, {msg}) => {
    const {horde} = msg.data;
    if (jitterTimeout[horde]) {
      clearInterval(jitterTimeout[horde]);
      delete attempt[horde];
    }
    onJitter(msg.data);
    jitterTimeout[horde] = setInterval(() => {
      attempt[horde] = attempt[horde] ? attempt[horde] + 1 : 1;
      onJitter({horde});
      if (attempt[horde] === 5) {
        onReconnect('attempt');
      }
  ...
  quest.goblin.defer(() => {
    for (const horde in jitterTimeout) {
      clearInterval(jitterTimeout[horde]);
      delete jitterTimeout[horde];
      delete attempt[horde];
    }
    unsub();
  });
...
Goblin.registerQuest(goblinName, 'delete', function (quest) {
  delete WINDOWS_REGISTRY[quest.goblin.id];
