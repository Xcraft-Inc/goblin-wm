...
Goblin.registerQuest(
  ...
  function*(quest, labId, url, options, next) {
    ...
    const onResend = watt(function*() {
      yield quest.warehouse.resend({feed: quest.goblin.id});
      onCommandsRegistry();
      const channel = quest.goblin.getX(`channel`);
    ...
    const getBrowserWindowOptions = () => {
      ...
      if (!winOptions.width && !winOptions.height) {
        ...
        winOptions.width = Math.floor(width);
        winOptions.height = Math.floor(height);
      }
      quest.log.info(
        `Browser window: ${winOptions.width}x${winOptions.height}`
      );
      // FIXME : Electron center the window given the screen size and not the work area
      return winOptions;
    };
...
Goblin.registerQuest(goblinName, 'begin-render', function*(quest) {
  yield quest.warehouse.syncChanges({feed: quest.goblin.id});
  const channel = quest.goblin.getX(`channel`);
