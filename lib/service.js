...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  const setLoginState = (state) => quest.goblin.setX('loginState', state);
  quest.goblin.defer(
    quest.sub(
  ...
  quest.goblin.defer(
    quest.sub.local(`*::${quest.goblin.id}.<run.requested>`, function* (
      ...
      try {
        ...
        _action.data._goblinUser = Goblin.buildGuestFootprint(clientSessionId);

        const loginState = getLoginState();
        if (loginState) {
          const {login, isLogged} = loginState;
          if (isLogged) {
            _action.data._goblinUser = `${login}@${clientSessionId}`;
          }
        }
