...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  const onLatency = (latency) => {
    ...
    if (channel) {
      let action;
      if (latency) {
        action = {type: 'PUSH_LATENCY', ...latency};
      } else {
        action = {type: 'NO_LATENCY'};
      }
      channel.sendAction(action);
    }
  };
  ...
  if (electronVibrancy && wmConfig.vibrancyOptions !== null) {
    win.setVibrancy(wmConfig.vibrancyOptions);
  }

  win.once('ready-to-show', () => {
    if (electronVibrancy && wmConfig.vibrancyOptions) {
      electronVibrancy.SetVibrancy(win, wmConfig.vibrancyOptions.material);
    }
    win.show();
  });
  ...
  let latencyTimeout;
  quest.goblin.defer(
    quest.sub.local('greathall::<latency>', (err, {msg}) => {
      if (latencyTimeout) {
        clearTimeout(latencyTimeout);
      }
      onLatency(msg.data);
      latencyTimeout = setTimeout(() => onLatency(null), 1000);
    })
  );
