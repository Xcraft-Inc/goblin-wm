...
Goblin.registerQuest(goblinName, 'capture-page-to-png', function*(
  quest,
  parameters,
  next
) {
  ...
  */
  ...
  return nativeImage;
  /*
  const dataUrl = nativeImage.toDataURL('image/png');
  const base64Data = dataUrl.replace(/^data:image\/png;base64,/, '');
  const fileName = 'image.png';
  let filePath = yield dialog.showSaveDialog(
    {defaultPath: fileName},
    next.arg(0)
  );
  if (filePath) {
    yield fs.writeFile(filePath, base64Data, 'base64', next);
  }
  */
});
