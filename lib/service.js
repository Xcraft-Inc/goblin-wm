...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  if (wmConfig.vibrancyOptions !== null) {
    //force frameless if vibrancy is set
    if (wmConfig.windowOptions === null) {
      wmConfig.windowOptions = {};
    }
    wmConfig.windowOptions.frame = false;
    try {
      BrowserWindow = require('electron-acrylic-window').BrowserWindow;
    } catch (ex) {
      /* Ignore vibrancy stuff on unsupported systems */
    }
  }
  ...
  const onLatency = (latency) => {
    const channel = quest.goblin.getX(`channel`);
    if (channel) {
      const action = {type: 'PUSH_LATENCY', ...latency};
      channel.sendAction(action);
    }
  };
