...
Goblin.registerQuest(
  ...
  function*(quest, labId, url, options, next) {
    ...
    quest.goblin.defer(
      quest.sub(`*::*.setWindow`, function(err, {msg, resp}) {
        if (msg.data.window) {
          const win = quest.goblin.getX('instances');
          win.setBounds(msg.data.window);
        }
      })
    );
    ...
    const getBrowserWindowOptions = () => {
      ...
      winOptions.webPreferences = {nodeIntegration: true};
    ...
    if (!options.useWS) {
      ...
      win.on('close', evt => {
        evt.preventDefault();
        quest.evt(`${labId}.window-closed`, {window: evt.sender.getBounds()});
      });
