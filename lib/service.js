...
function disposeChannel(quest) {
  quest.goblin.delX('channel');

  const sendNetworkStatus = quest.goblin.getX('sendNetworkStatus');
  if (sendNetworkStatus) {
    sendNetworkStatus.cancel();
  }
  const sendSyncingStatus = quest.goblin.getX('sendSyncingStatus');
  if (sendSyncingStatus) {
    sendSyncingStatus.cancel();
  }
...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  quest.goblin.setX('recording', false);
  ...
  quest.goblin.defer(
    quest.sub.local(`*::${quest.goblin.id}.<run.requested>`, function* (
      ...
      const isRecording = quest.goblin.getX('recording');
      if (isRecording) {
        pushAction(_action);
      }
...
Goblin.registerQuest(goblinName, 'feed-sub', function* (
  ...
  // sub to the desktop events
  const onDesktopEvent = (topic, data) => {
    const channel = quest.goblin.getX('channel');
    if (channel) {
      channel.sendEvent(topic, data);
    }
  };

  const unsub = quest.goblin.getX('desktopEventUnsub');
  if (unsub) {
    unsub();
  }
  quest.goblin.setX(
    'desktopEventUnsub',
    quest.sub.local(`greathall::<${desktopId}-event>`, (err, {msg}) => {
      const {topic, data} = msg.data;
      onDesktopEvent(topic, data);
    })
  );
...
Goblin.registerQuest(goblinName, 'start-recording', function (quest) {
  quest.goblin.setX('recording', true);
});

Goblin.registerQuest(goblinName, 'stop-recording', function (quest, savePath) {
  quest.goblin.setX('recording', false);
  fs.writeFileSync(savePath, JSON.stringify(RECORDED_ACTIONS, null, 2));
  RECORDED_ACTIONS = [];
});

Goblin.registerQuest(goblinName, 'delete', function (quest) {
  const desktopEventUnsub = quest.goblin.getX('desktopEventUnsub');
  if (desktopEventUnsub) {
    desktopEventUnsub();
  }
  ...
  const warehouseSUnsb = quest.goblin.getX('warehouseSub');
  const feedSubs = quest.goblin.getX('feedSubs');
