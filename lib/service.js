...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  //prepare a callback for waiting the window
  const windowCreatedCallback = next.parallel();
  const unsubLoad = quest.sub(
    `${quest.goblin.id}.<window-loaded>`,
    (...args) => {
      unsubLoad();
      return windowCreatedCallback(...args);
    }
  );

  const window = WM.create(quest.goblin.id, options);
  const win = window.win;
  quest.goblin.setX('instance', win);
  win.webContents.once('did-finish-load', () => {
    quest.evt(`<window-loaded>`);
  });
