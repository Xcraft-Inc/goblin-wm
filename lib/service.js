...
Goblin.registerQuest (goblinName, 'win.nav', function (quest, wid, route) {
...
Goblin.registerQuest (goblinName, 'win.delete', function (quest, wid) {
  const state = quest.goblin.getState ();
  const win = state.get (`instances.wid-${wid}`);
  if (win) {
    win.destroy ();
    delete fullStateNeeded[wid];
    delete previousStates[wid];
