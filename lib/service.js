...
const sendBackendState = (window, delta, transport) => {
  if (transport.name === 'electron') {
    transport.send ('NEW_BACKEND_STATE', transit.toJSON (delta), 'main');
    return;
  }
  ...
  if (transport.name === 'ws') {
    transport.send (
      JSON.stringify ({
        ...
        transitState: transit.toJSON (delta),
      })
    );
    return;
  }
...
Goblin.registerQuest (goblinName, 'list', function (quest) {
  ...
  quest.goblin.getX (`instances`).forEach ((v, k) => {
