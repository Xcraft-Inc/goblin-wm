...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  //FIXME: disable jitter
  quest.goblin.setX('jitterOn', false);
  quest.do();
  //Manage by feed subs
  ...
  const onReconnect = (status) => {
    ...
    if (channel) {
      ...
      if (status === 'attempt') {
        ...
        quest.goblin.setX('isDisconnected', true);
      } else {
        action.disconnected = false;
        action.message = '...';
        quest.goblin.setX('isDisconnected', false);
      }
      channel.sendAction(action);
    }
  };

  const onJitter = (payload) => {
    const channel = quest.goblin.getX(`channel`);
    if (!channel) {
      return;
    }

    /* FIXME: this jitterOn must be removed when we are able to use multi-stores
     * in the frontend in order to seperate the redux connect calls used only
     * for the infrastructure.
     */
    const jitterOn = quest.goblin.getX('jitterOn');
    const isDisconnected = quest.goblin.getX('isDisconnected');
    if (jitterOn) {
      channel.sendAction(
        payload.noJitter
          ? {type: 'NO_JITTER', ...payload}
          : {type: 'PUSH_JITTER', ...payload}
      );
    } else if (isDisconnected && !payload.noJitter) {
      /* Because the jitter is disabled in the frontend, we must remove the
       * overlay when appropriate. When jitterOn is false, this code is
       * useless because PUSH_JITTER reset the disconnected flag.
       */
      channel.sendAction({
  ...
  win.webContents.once('did-finish-load', onCommandsRegistry);
  ...
  quest.goblin.defer(
    quest.sub.local('greathall::<jitter>', (err, {msg}) => onJitter(msg.data))
  );
...
Goblin.registerQuest(goblinName, 'delete', function (quest) {
  /* Ensure that the channel is not longer available in case of
   * of race with a jitter events.
   */
