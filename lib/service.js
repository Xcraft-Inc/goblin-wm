...
Goblin.registerQuest(goblinName, 'create', function*(
  ...
  if (!options.useWS) {
    ...
    quest.goblin.setX(`transport-for-window`, new ElectronChannel(win));
    ...
    quest.goblin.setX(`transport-for-window`, new WebSocketChannel(win));
    quest.do({win});
  ...
  quest.goblin.defer(
    quest.sub(`*::warehouse.${winFeed}.changed`, (err, msg) => {
      if (win) {
        ...
        quest.goblin.getX(`transport-for-window`).sendBackendState(msg.data);
      }
    })
  );
...
Goblin.registerQuest(goblinName, 'nav', function(quest, route) {
  ...
  if (win) {
    ...
    quest.goblin.getX(`transport-for-window`).sendPushPath(route);
  }
});
...
Goblin.registerQuest(goblinName, 'dispatch', function(quest, action) {
  ...
  if (win) {
    ...
    quest.goblin.getX(`transport-for-window`).sendAction(action);
  }
});
